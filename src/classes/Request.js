...
const UNSPLASH_URL =
  "https://api.unsplash.com/search/photos/?client_id=9LhVwLjJrdIy5jX3svklsUACp0mByDjsrzbJNTZGAqg&query=";
...
export async function getCountries(params = null) {
  axios
    .get(`${COUNTRIES_URL}/all${params ? params : ""}`)
    .then((response) => {
...
export async function getNews(countryCode) {
  axios
    .get(`${NEWS_URL}${countryCode}${NEWS_KEY}`)
    .then((response) => {
      console.log(response.data);
      return response.data;
    })
...
export async function getEvents(countryCode) {
  axios
    .get(`${EVENTS_URL}${countryCode}${EVENTS_KEY}`)
    .then((response) => {
      console.log(response.data);
      return response.data;
    })
